...
<main id="primary" class="site-main fvph-single">
  <?php while(have_posts()): the_post();
    ...
    // Brand term
    $brand_terms = wp_get_post_terms(get_the_ID(), 'marca_equip', ['fields'=>'names']);
  ...
  <article <?php post_class('fvph-product'); ?>>
    <header class="fvph-hero">
      ...
      <div class="fvph-hero-meta">
        ...
        <div class="fvph-cta">
          <?php if($buy): ?><a class="fv-btn" href="<?php echo esc_url($buy); ?>" target="_blank" rel="nofollow sponsored noopener">Comprar na Tech Market Brasil</a><?php endif; ?>
        </div>
        <div class="fvph-meta-inline">
          <?php the_terms(get_the_ID(), 'marca_equip', '<span class="fv-meta-term">Marca: ', ', ', '</span>'); ?>
      ...
      <table class="fvph-specs-table">
        <tbody>
          <?php
            ...
            // Qualquer meta _fvph_attr_* adicional vira linha
            foreach(get_post_meta(get_the_ID()) as $k=>$vals){
                if(strpos($k,'_fvph_attr_')===0 && !in_array($k, ['_fvph_attr_puffs','_fvph_attr_nicotina','_fvph_attr_bateria'])){
                    ...
                    $rows[$label] = is_array($vals) ? implode(', ', $vals) : $vals;
                }
            }
            foreach($rows as $k=>$v){
                echo '<tr><th>'.esc_html($k).'</th><td>'.esc_html(is_array($v)?implode(', ',$v):$v).'</td></tr>';
            }
          ?>
        </tbody>
      </table>
    ...
    <footer class="fvph-footer">
      <div class="fvph-meta">
        <?php the_terms(get_the_ID(), 'categoria_equip', '<span class="fv-meta-term">Categorias: ', ', ', '</span>'); ?>
      </div>
      <nav class="fvph-nav">
        <a href="<?php echo esc_url(get_post_type_archive_link('equipamento')); ?>" class="fv-btn fv-btn-outline">Voltar para equipamentos</a>
      </nav>
    </footer>
  </article>
